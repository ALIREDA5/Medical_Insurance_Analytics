...
df = spark.table("medical_insurance.default.patient_feedback")
...
df = (
    df.transform(lambda d: remove_duplicates(d, ["feedback_id"]))
...
df.write \
  ...
  .format("delta") \
  .saveAsTable("medical_insurance.silver.patient_feedback_silver")
