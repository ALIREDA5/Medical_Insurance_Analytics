...
df = spark.table("medical_insurance.default.drug_inventory")
...
df = (
    df.transform(lambda d: remove_duplicates(d, ["inventory_id"]))
...
df.write \
  ...
  .format("delta") \
  .saveAsTable("medical_insurance.silver.drug_inventory_silver")