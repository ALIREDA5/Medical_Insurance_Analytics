...
df = spark.table("medical_insurance.default.medical_procedure")
...
df = (
    df.transform(lambda d: remove_duplicates(d, ["procedure_code"]))
...
df.write \
  ...
  .format("delta") \
  .saveAsTable("medical_insurance.silver.medical_procedure_silver")