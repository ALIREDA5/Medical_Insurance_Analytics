...
df = spark.table("medical_insurance.default.referral")
...
df = (
    df.transform(lambda d: remove_duplicates(d, ["referral_id"]))
...
df.write \
  ...
  .format("delta") \
  .saveAsTable("medical_insurance.silver.referral_silver")