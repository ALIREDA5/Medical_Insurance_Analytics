...
df = spark.table("medical_insurance.default.prescription_item")
...
df = (
    df.transform(lambda d: remove_duplicates(d, ["prescription_item_id"]))
...
df.write \
  ...
  .format("delta") \
  .saveAsTable("medical_insurance.silver.prescription_items_silver")