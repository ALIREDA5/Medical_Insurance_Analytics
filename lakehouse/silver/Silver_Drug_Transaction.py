...
df = spark.table("medical_insurance.default.drug_transaction")
...
df = (
    df.transform(lambda d: remove_duplicates(d, ["transaction_id"]))
...
df.write \
  ...
  .format("delta") \
  .saveAsTable("medical_insurance.silver.drug_transaction_silver")