...
df = spark.table("medical_insurance.default.visit_procedure")
...
df = (
    df.transform(lambda d: remove_duplicates(d, ["visit_procedure_id"]))
...
df.write \
  ...
  .format("delta") \
  .saveAsTable("medical_insurance.silver.visit_procedure_silver")